    ...
    <section>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <h2 className={`text-sm font-semibold ${openTickets.length === 0 ? 'text-muted' : 'text-gray-900'}`}>Active Tickets</h2>
        ...
        {openTickets.length === 0 ? (
          ...
          <>
            ...
            {openTickets.length > 1 && (
              ...
              >
                View all {openTickets.length} tickets <ChevronRight className="h-3.5 w-3.5" />
              </Link>
            )}
          </>
        )}
...
function TicketCard({ ticket, index }) {
  return (
    <motion.div
      ...
      whileHover={{ y: -2 }}
      className="relative overflow-hidden rounded-2xl bg-white shadow-card border border-gray-100/60 transition-shadow duration-200 hover:shadow-card-hover"
    >
          ...
          <div className="flex shrink-0 items-center gap-1.5 rounded-lg bg-warning-light px-2.5 py-1.5 mt-0.5">
            {ticket.eta ? (
              <>
                ...
                <span className="text-[11px] font-semibold text-warning whitespace-nowrap">Fix {ticket.eta}</span>
              </>
            ) : (
              <>
                <motion.span
                ...
                />
                <span className="text-[11px] font-medium text-warning whitespace-nowrap">Calculating</span>
              </>
            )}
          </div>
        </div>
